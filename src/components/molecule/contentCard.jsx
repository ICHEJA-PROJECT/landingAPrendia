...
const ContentCard = ({ urlImage, titulo, parrafo, alineacion = "imagen-texto", delay = 0 }) => {
  ...
  return (
    <section
      ref={ref}
      className={`flex flex-row items-center gap-x-16 w-full mb-20 transition-all duration-1000 ease-out ${!isImageFirst ? 'flex-row-reverse' : ''} ${
        isInView ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-12'
      }`}
      style={{ transitionDelay: `${delay}ms` }}
    >
      <div className="shrink-0">
        <img src={urlImage} alt={titulo} className="w-96 h-96 object-contain" />
      </div>
